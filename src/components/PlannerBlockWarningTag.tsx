...
export const PlannerBlockWarningTag = forwardRef<SVGSVGElement, PlannerBlockWarningTagProps>((props, ref) => {
    const [id] = useState(Guid.create().toString());
    const warningTag = `warning-tag_${id}`;
    const warningTagSign = `warning-tag-sign_${id}`;
    let warningTitle = '';
    if (props.errors && props.errors.length > 0) {
        warningTitle = props.errors[0];
        if (props.errors.length > 1) {
            warningTitle += ` and ${props.errors.length - 1} more errors. Click to view all.`;
        }
    } else {
        warningTitle = 'Block has issues. Click to view';
    }
    return (
        <>
            {props.show && (
                <foreignObject x="88" y="5" width={25} height={40}>
                    <Tooltip placement={'right'} title={warningTitle}>
                        <svg
                            x={0}
                            y={0}
                            ref={ref}
                            className={'warning-tag-container'}
                            fill="none"
                        ...
                        >
                            ...
                            <path
                                ...
                                x="20"
                                y="20"
                                d={SVGCornersHelper.createRoundedPathString(INITIAL_POINTS, 5)}
                            />
                            <svg id={warningTagSign} y="3" x="3" opacity="0" viewBox="0 0 16 16" overflow="visible">
                            ...
                            <animate
                                ...
                                attributeName="d"
                                from={SVGCornersHelper.createRoundedPathString(INITIAL_POINTS, 3)}
                                dur="0.2s"
                                fill="freeze"
                                scale="VERTICAL"
