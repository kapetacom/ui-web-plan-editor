...
@observer
export class PlannerBlockResourceListItem extends Component<
    PlannerBlockResourceListItemProps,
    PlannerBlockResourceListItemState
> {
    ...
    deleteHandler = async () => {
        const ok = await showDelete(
            'Delete resource',
            `Are you sure  you want to delete ${this.props.resource.getName()}`
        );
        if (!ok) {
            return;
        }
    ...
    render() {
        ...
        const counterValue = resourceConfig?.getCounterValue
            ? resourceConfig.getCounterValue(this.props.resource.getData())
            : 0;
